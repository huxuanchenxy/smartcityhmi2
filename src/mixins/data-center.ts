...
export const setDatavData = async (
  ...
  if(aConfig.isAppendDataList) {
    // 传入组件的数据
    ApiModule.addData({ comId, data: { [apiName]: res } });
  }
  else {
    // 传入组件的数据
  ...
  if (!DebugModule.debug) {
    if (com) {
      if (
        com.handles &&
        com.handles.refreshData &&
        com.handles.refreshData.fields &&
        com.handles.refreshData.fields.length > 0
      ) {
        //如果有载入事件
        com.handles.refreshData.fields.forEach(field => {
          field.value = res;
          ...
          if (field.hasDecideScript) {
            if (field.decideScript) {
              try {
                ...
                const decideRes: boolean = func(field.value, com);
                if (decideRes) {
                  // emit自定义事件
                  emitter.emit(field.targetComId, field);
                } else {
...
export const useDataCenter = (com: DatavComponent) => {
    ...
    for (const [name, ac] of Object.entries(apis.value)) {
      if (ac.useAutoUpdate && ac.autoUpdate > 0) {
        const timer = setInterval(
          <TimerHandler>(() => {
            setDatavData(com.id, name, ac, apiData.value[name], com);
          }),
          ac.autoUpdate * 1000
        );
  ...
  for (const [name, ac] of Object.entries(apis.value)) {
    ...
    watch(
      ...
      () => {
        setDatavData(com.id, name, ac, adc, com);
      },
      {
        deep: true,
        immediate: true
