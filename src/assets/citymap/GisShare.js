...
function setCamera(isAnimation, pointX, pointY, pointZ, headingValue, tiltValue, scaleValue){
    postToIframeMethed("setCamera",[isAnimation, pointX, pointY, pointZ, headingValue, tiltValue, scaleValue]);
}
function ViewChange() {
...
function doGetGraphicByJson(jsonStr,isGoTo,isPopup) {
...
window.addEventListener('message',function(e){
    ...
    switch(data.method){
        ...
        case "mapOnBuildClickCallback":{
            mapOnBuildClickCallback(data.data[0],data.data[1]);
            break;
        }
...
function mapOnclickCallback(data, gridid){
	console.log(data+"网格编码="+gridid)
     //alert(data+"网格编码="+gridid);
}
function mapOnBuildClickCallback(data,positionPoint) {
    console.log(data)
...
function doOnLightArea(layerName, where, colorList, orderField){
    postToIframeMethed("highLightArea", [layerName, where, colorList, orderField])
}

function doMakeCircleByData(lng,lat,radius,fillColor,borderColor,fillTransparency,outlineWidth){
    postToIframeMethed("makeCircleByJson", [lng,lat,radius,fillColor,borderColor,fillTransparency,outlineWidth])
}

function doPopupIn(popupInfoArray){
    postToIframeMethed("onPopupIn",[popupInfoArray]);
}
...
function doMakeHeatmap(data,colorStops,blurRadius,maxPixelIntensity,minPixelIntensity,field) {
    postToIframeMethed("makeHeatmap",[data,colorStops,blurRadius,maxPixelIntensity,minPixelIntensity,field]);
}


function doRemoveHeatmap() {
    postToIframeMethed("removeHeatmap");
}

function doAddDustbin(point) {
    postToIframeMethed("addDustbin",[point]);
}

function doQuery3DBuilding(code) {
    postToIframeMethed("query3DBuilding",[code]);
}
function doColor3DBuilding(code,symbol) {
    postToIframeMethed("color3DBuilding",[code,symbol]);
}
function doGotoPositionByObjectID(code) {
    postToIframeMethed("gotoPositionByObjectID",[code]);
}
function doInitBuild() {
    postToIframeMethed("doInitBuild");
}
function doGetCamera() {
    postToIframeMethed("getCamera");
}
