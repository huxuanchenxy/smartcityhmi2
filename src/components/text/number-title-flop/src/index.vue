<template>
  <div class="datav-wrapper" :style="wrapperStyle">
    ...
    <div :style="counterStyle">
      ...
      <span v-for="(n, i) in realNumber" :key="i" :style="[n === separatingSymbol ? separateCharStyle : numberStyle]">
        {{ n }}
      </span>
...
<script lang='ts'>
import { defineComponent, PropType, ref, toRef, computed, watchEffect, getCurrentInstance } from 'vue'
...
import { calcStrWidth, setNP } from '@/utils/util'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const wrapperStyle = computed(() => {
      ...
      style['cursor'] = config.value.cursor
      return {
    ...
    const numberStyle = computed(() => {
      ...
      const style: Partial<CSSProperties> = {
        ...
        textAlign: numbers.textalign as any,
      }
    ...
    watchEffect(() => {
      const { numbers } = config.value
      let num: number = config.value.numbers.icValue ? config.value.numbers.icValue : (dv_data.value[dv_field.value.value] || 0)
