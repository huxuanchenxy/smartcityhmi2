<template>
  <div class="datav-wrapper" :style="wrapperStyle">
    <svg v-if="config.iconStyle.show" width="18px" height="18px" viewBox="0 0 18 18" version="1.1"
      xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" :style="iconStyle">
      <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
        <g transform="translate(-920.000000, -643.000000)" :fill="config.iconStyle.color"
    ...
    <span v-if="config.time.show">{{ realDateTime }}</span>
    <div v-if="config.week.show" :style="weekStyle">{{ realWeek }}</div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const weekStyle = computed(() => {
      const { week } = config.value
      return {
        marginLeft: `${week.marginLeft}px`,
      }
    })
    ...
    const realWeek = computed(() => {
      return dayjs(dateTime.value).locale('zh-cn').format("dddd")
    })
