<template>
  ...
  <div :id="editorId" class="datav-editor" :class="[{ '--read-only': readOnly, }, editorClass]">
    <div v-if="!loading" class="datav-editor-actions">
      <n-icon class="action-btn" title="点击复制" @click="copyData">
        <IconCopy />
      </n-icon>
      <n-icon class="action-btn" :title="isFullScreen ? '退出全屏' : '全屏模式下编辑或查看'" @click="switchFullScreen">
  ...
  <n-modal v-model:show="isFullScreen" :title="`${fullScreenTitle}${readOnly ? ' ( 只读 )' : ''}`" preset="dialog"
    :show-icon="false" :mask-closable="false" class="datav-fullscreen-editor-dialog" style="width: 90%; margin-top: 1%;"
    @after-leave="closedFullScreenDialog" :autoFocus="false">
    <div class="datav-fullscreen-editor-wp">
      <div class="datav-editor datav-fullscreen-editor" :class="[{ '--read-only': readOnly, }]">
