  ...
  <div class="setting-panel-gui">
    ...
    <g-field-collapse label="工控">
      ...
      >
        <n-input-group>
          <n-input
            v-model:value="config.control.nodeCode"
...
<script lang='ts'>
import { defineComponent, PropType, toRef, computed,ref ,provide } from 'vue'
import {
  fontFamilys,
  selectSuggests,
...
import { DynamicSingleBar, DynamicSingleSeries } from './dynamic-single-bar'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const handleAddSeriesItem = () => {
      return new DynamicSingleSeries(`系列${config.value.series.length + 1}`)
    }
    ...
    provide('addPt', (item: PtModel) => {
      if (item != null && item.extension && item.extension.device && item.extension.name) {
        config.value.control.deviceCode = item.extension.device
        config.value.control.nodeCode = item.extension.name
