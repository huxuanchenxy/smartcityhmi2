...
import { DrillDownBar } from './drill-down-bar'
...
export default defineComponent({
  ...
  props: {
    com: {
      type: Object as PropType<DrillDownBar>,
      required: true,
    },
  },
  setup(props) {
    ...
    const chartData = computed(() => {
      const groups = groupBy(dv_data.value, item => item[dv_field.value.z])
      return {
    ...
    const getXData = () => {
      ...
      if (values[datacategory.value]) {
        return values[datacategory.value].map(r => r.x)
      }
      else {
        return []
      }
    ...
    const getSeries = () => {
      ...
      return series.map((item, idx) => {
        ...
        return {
          ...
          data: map(values[datacategory.value], v => {
            const obj = v
            return {
    ...
    const option = computed(() => {
      ...
      let graphicList = [{
        ...
        left: 50,
        top: 20,
        invisible: datacategory.value == 0,
        style: {
          text: '回退',
          fontSize: 18,
          fill: 'white',
      ...
      const opts = {
        ...
        xAxis: {
          ...
          data: getXData(),//seriesList[datacategory.value].data.map(r=>r.name),//chartData.value.keys,
        },
        yAxis: {
          show: yAxis.show,
          type: 'value',
    ...
    const onClick = (params: any) => {
      ...
      let index = keys.findIndex(r => r == params.data.name)
      ...
      datacategory.value = index
      // optionValue.series[0].data=data
      // optionValue.xAxis.data=data.map(r=>r.label)
    }
