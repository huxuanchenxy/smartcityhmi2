<template>
  <div class="setting-panel-gui">
    ...
    >
      <template #default="slotProps">
        ...
        >
          <n-input-group>
            <n-input
              v-model:value="slotProps.item.ic.nodeCode"
            ...
            <n-button
              ...
              size="small"
              ghost
              @click="openIcList(slotProps.item)"
            >
              点位
            </n-button>
          </n-input-group>
    ...
    <g-field-collapse label="工控">
      <g-field
        :level="2"
        label="间隔时间"
      >
        <g-input-number
          v-model="apis.source.autoUpdate"
          :min="0"
          :step="1"
          suffix="秒"
        />
      ...
      >
        <g-input-number
          v-model="config.control.startCount"
          :min="0"
          :step="1"
        />
      </g-field>
      ...
      >
        <g-select
          v-model="config.control.interval"
          :data="[{ id: '1分钟', value : 60 },{ id: '5分钟', value : 60 }]"
        />
      </g-field> -->
    </g-field-collapse>
  </div>
...
<script lang='ts'>
import { defineComponent, PropType, toRef,ref ,provide } from 'vue'
import {
  fontFamilys,
  axisTypes,
  selectSuggests,
...
import { DynamicBar, DynamicBarSeries } from './dynamic-bar'
import IcList from '@/views/screen-editor/config-panel/icevent-panel/ic-list.vue'
import { IcHandleItemConfig, IcHandleItemField, PtModel } from '@/components/data-handle'
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const apis = toRef(props.com, 'apis')
    const apiData = toRef(props.com, 'apiData')

    const icListVisibile = ref(false)

    let currentSeries:DynamicBarSeries = null

    const handleAddSeriesItem = () => {
      return new DynamicBarSeries('')
    }

    const openIcList = (item:DynamicBarSeries) => {
      currentSeries = item
    ...
    provide('addPt', (item: PtModel) => {
      if (item != null && item.extension  && item.extension.device && item.extension.name) {
        ...
        currentSeries.ic.deviceCode = item.extension.device
        currentSeries.ic.nodeCode =  item.extension.name;
        icListVisibile.value = false

        const apisConfig = (apis.value as ApiConfigMap).source
        const apiDataConfig = (apiData.value as ApiDataConfigMap).source
        if(!apisConfig.useAutoUpdate && !apiDataConfig.config.api)
        {
          apiDataConfig.type = ApiType.api
          apisConfig.useAutoUpdate = true
          ...
          apisConfig.fields['x'].map = 'seq'
          apisConfig.fields['y'].map = 'value'
          apisConfig.fields['s'].map = 'nodeCode'
          //apiDataConfig.config.useFilter = true
          //apiDataConfig.pageFilters.
        }
      }
    })
