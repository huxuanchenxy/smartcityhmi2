...
<script lang='ts'>
import { defineComponent, PropType, computed, toRef, ref, watch, getCurrentInstance } from 'vue'
    ...
    mitter.on(props.com.id, (field: HandleItemField) => {
      switch (field.targetMethodName) {
        ...
        case 'show':
          ...
          break
        case 'initData':
          props.com.apiData.source.type = ApiType.static
          props.com.apiData.source.config.data = field.params
          break
        case 'initConfig': //初始化组件配置
          const func = new Function(field.script)
          console.log('初始化组件配置',config.value, func())
          config.value = func()
          break
        case 'filter':
        ...
        case 'custom':
          try {
            const func = new Function('data,getLocalValue,setLocalValue', field.script)
            func(field.params, LocalValueModule.getLocalData,LocalValueModule.setLocalData)
          } catch (error) {
