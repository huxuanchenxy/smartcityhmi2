<template>
  <div class="setting-panel-gui">
    ...
    >
      ...
      >
        <g-input-number
          v-model="config.xAxis.splitNumber"
          :min="0"
          :max="100"
          :step="1"
        />
...
import { BasicHorizontal, BasicHorizontalSeries } from './basic-horizontal'

export default defineComponent({
  name: 'VBasicHorizontalProp',
  props: {
    com: {
  ...
  setup(props) {
    ...
    const yAxisTypes = computed(() => {
      return axisTypes.filter(m => m.id !== 'value')
    })

    const handleAddSeriesItem = () => {
      return new BasicHorizontalSeries(`系列${config.value.series.length + 1}`)
    }
