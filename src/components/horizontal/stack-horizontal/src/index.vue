...
const props = defineProps({
  com: {
    type: Object as PropType<StackHorizontal>,
    required: true,
  },
})
...
const getSeries = (keys: string[]) => {
  const { global, xAxis, series } = config.value
...
const option = computed(() => {
  ...
  if (keys.length > 0) {
    data = chartData.value[keys[0]].map(m => m[dv_field.value.y])
  }
  const opts = {
    textStyle: {
