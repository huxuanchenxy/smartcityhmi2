<template>
  <div class="canvas-main">
    <div id="canvas-wp" class="canvas-panel-wrap" @mousedown.stop="cancelSelectCom" @mousedown="handleMouseDown">
      <div id="screen-wp" class="screen-shot" :style="screenShotStyle">
        <align-line />
        <ruler />
        <div id="canvas-coms" class="canvas-panel" :style="canvasPanelStyle" @dragover="dragOver" @drop="dropToAddCom">
          <datav-transform v-for="com in coms" :key="com.id" :com="com">
            <component :is="com.name" :com="com" :style="{
      ...
      <canvas-area v-if="areaWidth > 0 || areaHeight > 0" :start-x="areaStartX" :start-y="areaStartY" :width="areaWidth"
        :height="areaHeight" :style="{
          width: `${selectionW}px`,
          height: `${selectionH}px`,
        }" />
    </div>
    <div id="thumbnail" class="thumbnail" :style="{ display: eyeView ? 'inline' : 'none' }"
      @mousewheel.prevent="zoomEyeView">
      <div class="datas-thumbnail">
        <!-- <cavans></cavans> -->
        <span id="select-span" :style="selectSpanStyle" @mousedown.prevent.stop="eyeMove"></span>
      </div>
    </div>
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const selectSpanStyle = computed(() => {
      ...
      var span_width = Math.round(view_width / (canvas.value.scale * pageConfig.value.width + 100) * 190);
      var span_height = Math.round(view_height / (canvas.value.scale * pageConfig.value.height + 100) * 110);
    ...
    const dropToAddCom = async (event: any) => {
      ...
      try {
        ...
        if (name) {
          ...
          const offsetX = (event.clientX - ToolbarModule.getPanelOffsetLeft + EditorModule.eyeView.x) / scale
          const offsetY = (event.clientY - ToolbarModule.getPanelOffsetTop + EditorModule.eyeView.y) / scale
          //console.log(offsetX);
          //console.log(offsetY);
    ...
    //监控滚动条的值来对鹰眼框进行同步
    watch(() => EditorModule.eyeView, (eyeView) => {
      const { scale } = canvas.value;
      ...
      span.style.transform = 'translate(' + current_x + 'px,' + current_y + 'px)';
    }, { deep: true })
