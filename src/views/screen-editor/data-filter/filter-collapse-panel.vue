<template>
  <div ref="panelRef" class="collapse-panel-wp filter-item ds-panel-filter" :class="{
    '--create': isNew,
      ...
      <div class="panel-title">
        <div class="filter-title">
          <template v-if="draggable">
            <n-icon draggable="true" class="drag-btn" @dragstart="dragStart" @dragend="dragEnd">
              <IconDrag />
            </n-icon>
            <n-checkbox :checked="isEnabled" class="enable-checkbox" @update:checked="switchEnabled" />
          </template>
          <div class="filter-name">
            <new-input v-if="editing" :model-value="dataFilter.name" :autofocus="true" class="filter-name-input"
              @change="editName" />
          ...
          <div class="filter-count">
            <template v-if="!isNew && usedFilters[dataFilter.id]">
              <n-tooltip placement="top" :delay="500" :style="{
                '--padding': '6px',
                '--border-radius': '2px',
                '--text-color': '#000102',
                '--arrow-height': '8px',
          ...
          <div class="action-wp" :class="{ '--flex': publish }">
            ...
            <g-tooltip-popover content="删除">
              <n-icon class="del-btn" @click="removeFilter(dataFilter.projectId, dataFilter.id)">
          ...
          <template v-if="hasFeedback">
            <template v-if="errMsg">
              <n-tooltip placement="left" :style="{
      ...
      <n-collapse-transition :collapsed="collapse">
        <div class="panel-content-wp">
          <div class="cp-wrap">
            ...
            <g-monaco-editor language="javascript" :code="code" :height="180" editor-class="filter-editor"
              @change="changeCode" @blur="updateCode" />
            ...
            <div class="filter-actions">
              ...
              <n-button size="tiny" :focusable="false" class="bolder-btn" @click="cancelEdit">
                {{
                  dataFilter.id === 0 ? '取消' : isEdited ? '撤销' : '取消'
                }}
              </n-button>
              <n-button type="primary" size="tiny" :loading="loading" :focusable="false" class="bolder-btn"
                @click="saveData">
                {{
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const removeFilter = inject('removeFilter') as (projectId: number, id: number) => void
    ...
    const saveData = async () => {
      ...
      try {
        loading.value = true
        props.dataFilter.projectId = EditorModule.screen.id;
