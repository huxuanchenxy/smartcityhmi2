<template>
  ...
  >
    <n-drawer-content closable>
      ...
      <ds-from-parent-editor v-else-if="apiDataConfig.type === apiType.fromParent" />
...
export default defineComponent({
  ...
  components: {
    ...
    DsFromParentEditor: loadAsyncComponent(() => import('./api-editors/ds-parent-editor.vue')),
  },
  setup() {
    ...
    const changeSource = (val: ApiType) => {
      switch (val) {
        case ApiType.api:
          ...
          break
        case ApiType.fromParent:
          apiDataConfig.value.config.parentData = ApiModule.dataMap[com.value.parentId]?.source ?? []
          break
        default:
          break
      }
