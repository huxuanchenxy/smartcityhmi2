<template>
  ...
  >
    <div class="create-dialog">
      <p class="name-title"><span class="required">*</span>应用名称</p>
      <new-input
        v-model="projectName"
        placeholder="请输入应用名称"
        spellcheck="false"
        class="name-input"
      />
      <p class="name-title">应用分组</p>
      <n-select v-model:value="groupId" :options="groupOpts" size="small" />
    </div>
    <template #action>
      <n-button :focusable="false" @click="visibleCreateDialog = false">
        取消
      </n-button>
  ...
  setup() {
    ...
    onMounted(async () => {
      ...
      getProjects().then(({ data }) => {
        if (data.code === 200) {
          groups.value.push(
            ...data.data.map(({ id, name }: any) => ({ id, name }))
          );
        }
      });
    });
  ...
  .creator-wp {
    ...
    .template-list {
      ...
      .template-size {
        color: #999;
        text-align: right;
        font-family: Orbitron-Bold, PingFangSC-Medium, PingFang SC,
