  ...
  <div class="login-container">
    ...
    <n-form ref="loginFormRef" :model="loginForm" :rules="loginRules" label-placement="left" size="small"
      ...
      <n-form-item path="username">
        <n-input v-model:value="loginForm.username" placeholder="用户名" type="text" size="large" :style="inputCSSVars">
          <template #prefix>
            <n-icon>
              <IconUser />
            </n-icon>
        ...
        <n-tooltip :show="capsTooltip" placement="top-start">
          <template #trigger>
            <n-input v-model:value="loginForm.password" placeholder="请输入密码" type="password" size="large"
              show-password-on="click" :style="inputCSSVars" @keydown="checkCapslock" @blur="capsTooltip = false"
      ...
      <n-button :loading="loading" type="primary" size="large" style="width: 100%; margin-bottom: 20px;"
        @click="handleLogin">
        {{ $t('login.login') }}
      </n-button>
