...
class Project extends VuexModule implements IProjectState {
  ...
  private DELETE_PROJECT(payload: { pid: number; gid: number }) {
    const g = this.allGroups.find(m => m.id === -1);
    if (g) {
  ...
  public async getProjects() {
    try {
      const res = await api.getProjectsByGroup(getProjectId());
      if (res.data.code === 200) {
        // const list: ProjectGroup[] = res.data.data || []
        // this.SET_PROJECTS(list)
        const list = [new ProjectGroup(-1, "全部应用", res.data.data || [])];
        this.SET_PROJECTS(list);
      } else {
  ...
  public async moveProject(payload: { pid: number; fromId: number; toId: number }) {
    try {
      //const { pid, fromId, toId } = payload
      const res = await api.moveProject({ id: payload.pid, groupId: payload.toId });
      if (res.data.code === 200) {
  ...
  public async createProjectGroup(name: string) {
    try {
      const { data } = await api.createProjectGroup({ groupName: name });
      if (data.code === 200) {
  ...
  public async deleteProjectGroup(id: number) {
    try {
      const res = await api.deleteProjectGroup({ id: id });
      if (res.data.code === 200) {
  ...
  public async updateProjectGroupName(payload: { id: number; newName: string }) {
    try {
      const res = await api.updateProjectGroupName({ id: payload.id, groupName: payload.newName });
      if (res.data.code !== 200) {
        throw Error(res.data.message);
      }
    } catch (error) {
